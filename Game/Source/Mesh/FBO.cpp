...
FBO::FBO(unsigned int width, unsigned int height, int minFilter, int magFilter, bool depthReadable)
{
...
void FBO::Setup(unsigned int width, unsigned int height, int minFilter, int magFilter, bool depthReadable)
{
    assert( width <= 4096 );
    assert( height <= 4096 );
...
void FBO::OnSurfaceChange(ivec2 newWindowSize)
{
	Setup(newWindowSize.x, newWindowSize.y, m_MinFilter, m_MagFilter, m_DepthIsTexture);
}

void FBO::Invalidate(bool cleanGLAllocs)
{
